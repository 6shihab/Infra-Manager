...
export function saveSession(db: Database, session: CachedSession): void {
    db.run(
        `INSERT OR REPLACE INTO _cached_session (id, user_id, email, full_name, is_superuser, totp_enabled, has_passkeys, token, cached_at)
         VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [session.user_id, session.email, session.full_name, session.is_superuser ? 1 : 0, session.totp_enabled ? 1 : 0, session.has_passkeys ? 1 : 0, encrypt(session.token), session.cached_at]
    );
    saveDb();
}

export function getSession(db: Database): CachedSession | null {
    const result = db.exec('SELECT user_id, email, full_name, is_superuser, totp_enabled, has_passkeys, token, cached_at FROM _cached_session WHERE id = 1');
    if (result.length === 0 || result[0].values.length === 0) return null;
    const row = result[0].values[0];
    const rawToken = row[6] as string;
    return {
