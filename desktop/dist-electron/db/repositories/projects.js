...
exports.upsertProjectServerLink = upsertProjectServerLink;
exports.upsertProjectDatabaseLink = upsertProjectDatabaseLink;
exports.upsertProjectGroupAccess = upsertProjectGroupAccess;
exports.upsertProjectUserAccess = upsertProjectUserAccess;
exports.clearProjectLinks = clearProjectLinks;
exports.deleteProjectServerLink = deleteProjectServerLink;
exports.deleteProjectDatabaseLink = deleteProjectDatabaseLink;
const index_1 = require("../index");
const crypto = __importStar(require("crypto"));
function generateUUID() {
    return crypto.randomUUID();
}
function rowToProject(columns, row) {
    const obj = {};
    ...
    if (comps.length > 0) {
        ...
        project.components = comps[0].values.map(row => {
            ...
            if (typeof comp.custom_fields === 'string') {
                try {
                    comp.custom_fields = JSON.parse(comp.custom_fields);
                }
                catch {
                    comp.custom_fields = {};
                }
...
function createProject(db, data) {
    ...
    db.run(`INSERT INTO projects (id, name, description, primary_domain, environment, deployment_note, is_deleted, created_by, _is_local, _local_updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 0, ?, 1, ?)`, [id, data.name, data.description || null, data.primary_domain || null, data.environment || 'Dev', data.deployment_note || null, data.created_by || null, now]);
    (0, index_1.saveDb)();
...
function softDeleteProject(db, id) {
    const now = new Date().toISOString();
...
function upsertProject(db, project) {
    db.run(`INSERT OR REPLACE INTO projects (id, name, description, primary_domain, environment, deployment_note, is_online, last_checked_at, is_deleted, deleted_at, created_by, server_count, database_count, current_user_role, _is_local, _local_updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NULL)`, [
...
function upsertProjectServerLink(db, projectId, link) {
    db.run(`INSERT OR REPLACE INTO project_server (project_id, server_id, username, password, ssh_key)
         VALUES (?, ?, ?, ?, ?)`, [projectId, link.server_id || link.server?.id, link.username || null, link.password || null, link.ssh_key || null]);
}
function upsertProjectDatabaseLink(db, projectId, link) {
    db.run(`INSERT OR REPLACE INTO project_database (project_id, database_engine_id, db_name, username, password)
         VALUES (?, ?, ?, ?, ?)`, [projectId, link.database_engine_id || link.database_engine?.id, link.db_name || '', link.username || null, link.password || null]);
}
function upsertProjectGroupAccess(db, access) {
    db.run(`INSERT OR REPLACE INTO project_group_access (id, project_id, group_id, access_level)
         VALUES (?, ?, ?, ?)`, [access.id, access.project_id, access.group_id, access.access_level || 'Viewer']);
}
function upsertProjectUserAccess(db, access) {
    db.run(`INSERT OR REPLACE INTO project_user_access (id, project_id, user_id, access_level)
         VALUES (?, ?, ?, ?)`, [access.id, access.project_id, access.user_id, access.access_level || 'Viewer']);
}
function clearProjectLinks(db, projectId) {
...
function deleteProjectServerLink(db, projectId, serverId) {
    db.run('DELETE FROM project_server WHERE project_id = ? AND server_id = ?', [projectId, serverId]);
    (0, index_1.saveDb)();
}
function deleteProjectDatabaseLink(db, projectId, dbEngineId) {
